...
_base_corpus_dir = "/home/sim/Datasets/twitter2/data/lxmls"
def build_dicts():
    '''Builds feature dictionaries for a given corpus.''' 
    feat_counts = {}

    ### Build Dictionarie wit counts
    nr_pos = 0
    pos_file = open(path.join(_base_corpus_dir,"20121011_counts_positive_equal.lxmls"))
    for line in pos_file:
        nr_pos += 1
        toks = line.split(" ")
        for feat in toks[0:-1]: #simon: changed the index to start at 0
    ...
    pos_file.close()
    nr_neg = 0
    neg_file = open(path.join(_base_corpus_dir,"20121011_counts_negative_equal.lxmls"))
